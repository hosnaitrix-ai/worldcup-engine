...
if not df_future.empty:
    ...
    for _, r in df_future.iterrows():
        ...
        home, away, liga_corrente = r["Home"], r["Away"], r["League"]
        ...
        if ah == 1.0 and aa == 1.0: xg_h += 0.04; xg_a -= 0.04
        ...
        saida.append({
            ...
            "RawDate": r["Date"], "Date": r["DateStr"], "Time": r["Time"], "Home": home, "Away": away, "League": liga_corrente,

            "💡 Sugestão Value": sugestao_value, "anytime_label": lbl_anytime, "anytime_prob": prob_anytime, "anytime_odd_justa": odd_justa,
    ...
    if datas_disponiveis:
        ...
        if not df_hist_filtrada.empty: media_liga = df_hist_filtrada["TOTALGOALS"].mean()

        else: media_liga = LIGAS_MAPA.get(liga_selecionada, {}).get("base_home", 1.5) + LIGAS_MAPA.get(liga_selecionada, {}).get("base_away", 1.1)
        ...
        ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False); ax.spines['left'].set_color('#334155'); ax.spines['bottom'].set_color('#334155')

        ax.tick_params(axis='both', colors='#94A3B8', labelsize=9); ax.grid(axis='y', linestyle=':', alpha=0.2, color='#94A3B8')

        for bar in bars:

            height = bar.get_height()

            ax.annotate(f'{height:.2f}', xy=(bar.get_x() + bar.get_width() / 2, height), xytext=(0, 4), textcoords="offset points", ha='center', va='bottom', fontsize=9, color='#06B6D4', fontweight='bold')
        ...
        st.info("Buscando tabelas atualizadas. Caso as ligas não possuam rodadas futuras, serão exibidas assim que o calendário for publicado.")
...
with st.expander("🗂️ Central de Liquidez e Banco de Dados Histórico Online"):
    ...
    with col1: st.markdown(f'<div class="metric-card"><div class="metric-title">Amostragem Base</div><div class="metric-value">{len(df_hist_view)} Jogos</div></div>', unsafe_allow_html=True)

    with col2: 

        media_g_hist = df_hist_view["TOTALGOALS"].mean() if not df_hist_view.empty else 0.0

        st.markdown(f'<div class="metric-card"><div class="metric-title">Média de Gols</div><div class="metric-value">{media_g_hist:.2f}</div></div>', unsafe_allow_html=True)

    with col3: 

        pct_over = (df_hist_view["TOTALGOALS"] > 2.5).mean() * 100 if not df_hist_view.empty else 0.0

        st.markdown(f'<div class="metric-card"><div class="metric-title">Liquidez Over 2.5</div><div class="metric-value">{pct_over:.1f}%</div></div>', unsafe_allow_html=True)
